def bfs(root_node):
    '''
    :param root_node: 너비우선 탐색을 시작할 서브트리의 루트
    :return: 완성된 경로
    '''
    ...
    while data_structure:   # 후보군이 남아있으면 계속 조사
        # node = data_structure.pop() # LIFO 해버려서 DFS가 돼버렸다..
        node = data_structure.pop(0)    # FIFO
        result.append(node)
        # 내 자식 노드들 (인접 노드들)을 모두 찾아서
        for child in graph.get(node, []):   # get은 찾는 키가 없으면 None을 반환하나, default값 설정 가능
