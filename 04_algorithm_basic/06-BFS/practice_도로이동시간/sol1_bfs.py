...
def get_road_move_time(row, col):
    ...
    while queue:    # 후보군이 있는 동안
        ...
        for k in range(4):
            ...
            # 그러려면 리스트의 범위를 벗어나지 않아야 한다.
            # 그리고, 이전에 방문한 적이 없어야 한다. -> -1로 초기화 해 두었음
            ...
            if 0 <= nx < N and 0 <= ny < M and distance[nx][ny] == -1 and data[nx][ny]:
                ...
                # 도착지점에 도착하면, BFS 특성 상 가장 빠르게 도착한 길이니
                # 그때까지의 비용을 할당하고 종료
                if nx == N - 1 and ny == M - 1: # 도착지다.
                    return
    # 모든 후보군을 다 탐색했지만, return 돼서 함수가 종료된 적이 없이
    # 코드가 이 곳까지 도달했다면? 도착할 수 없다는 의미다.
    return -1
