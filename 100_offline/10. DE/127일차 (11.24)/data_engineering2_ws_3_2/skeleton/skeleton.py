...
with DAG(
    ...
    tags=['test'],  # DAG 태그 (Airflow UI에서 관리 용이) - 원하는 내용 입력
) as dag:

    # DAG 시작과 끝을 나타내는 빈 태스크 생성
    start = EmptyOperator(task_id="sijak")  # 시작 태스크
    check_scheduler = EmptyOperator(task_id="schedule")  # 스케줄러 확인 태스크
    check_worker = EmptyOperator(task_id="work")  # 워커 확인 태스크
    end = EmptyOperator(task_id="jongryo")  # 종료 태스크
