...
with DAG(
    ...
    tags=["test_web_ui"],  # DAG 태그 (Airflow UI에서 필터링 및 관리할 때 유용)
) as dag:

    # DAG 실행 시작을 나타내는 태스크 (실행만 하고 아무 동작 없음)
    start = EmptyOperator(task_id="sijak")  

    # 병렬 실행을 테스트하는 태스크 (start_task 실행 후 동시에 실행됨)
    parallel_1 = EmptyOperator(task_id="par1")  # 병렬 태스크 1
    parallel_2 = EmptyOperator(task_id="par2")  # 병렬 태스크 2

    # 병렬 실행 후 합쳐지는 태스크 (두 개의 병렬 태스크가 완료된 후 실행됨)
    after_parallel = EmptyOperator(task_id="after")

    # 순차 실행을 테스트하는 태스크 (순서대로 실행됨)
    sequential_1 = EmptyOperator(task_id="seq1")  # 순차 실행 태스크 1
    sequential_2 = EmptyOperator(task_id="seq2")  # 순차 실행 태스크 2

    # DAG 실행 종료를 나타내는 태스크 (모든 실행이 끝난 후 마지막으로 실행됨)
    end = EmptyOperator(task_id="jongryo")  
