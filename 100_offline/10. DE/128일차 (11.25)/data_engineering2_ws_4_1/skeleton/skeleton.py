...
with DAG(
    dag_id="test",  # DAG의 고유한 식별자 (이름)
    schedule="0 0 * * *",  # DAG의 실행 일정 (매일 00:00 실행)
    start_date=pendulum.datetime(2025, 11, 25, tz="Asia/Seoul"),  # DAG의 시작 날짜 (빈칸 채우기)
) as dag:
    
    # 첫 번째 BashOperator 정의
    bash_t1 = BashOperator(
        task_id="bash_t1",  # 태스크의 고유 식별자
        bash_command="echo whoami",  # 실행할 Bash 명령어 (현재 실행 중인 사용자 출력)
    )
