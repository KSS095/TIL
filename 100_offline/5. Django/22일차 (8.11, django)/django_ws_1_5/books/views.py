...
def book_list(request):
    if request.method == 'POST':  # 새 책을 추가하는 경우
        required_fields = ['title', 'description', 'published_date']
        missing_fields = []
        data = request.data
        ...
        if missing_fields:
            return JsonResponse(
                {'error': f'Field(s) {", ".join(missing_fields)} is required'},
                status = 400
            )
        
        date_pattern = re.compile(r'^\d{4}-\d{2}-\d{2}$')
        if not date_pattern.match(data['published_date']):
            return JsonResponse(
    ...
    page = int(request.query_params.get('page', 1)) # 페이지 번호를 가져옴 (기본값: 1)
