...
'''
    ...
    그건 그거고, Serializer의 원래 역할이 뭔데?
    어떤 data를 formatting 하는게 원래 역할.
'''

# class SomSerializer(serializers.Serializer):
#     # 얘는 모델에 대한 정보를 참고할 수 없음
#     # 그럼 뭘 포맷팅 한다는거임?
#     # 그 포맷팅 할 대상 데이터가 어떤 형태일건지 여기서 정의 하셈
...
class AuthorSerializer(serializers.ModelSerializer):
    # Meta class 내에 있는 변수랑 밖에 있는 변수에 어떤 차이가 있나요?
    ...
    def get_book_count(self, obj):
        '''
            obj: 이 시리얼라이저를 호출한 객체 (아마 author 일 듯)
        '''
