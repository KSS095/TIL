...
지하철 api 사이트: https://data.seoul.go.kr/dataList/OA-12914/S/1/datasetView.do
버스 api 사이트: https://data.seoul.go.kr/dataList/OA-12912/S/1/datasetView.do
...
BUS_API_KEY = "61756b6b41746d643937447a4a7068"  # 서울시 버스 교통 데이터 API 키 (빈칸 채우기)
SUBWAY_API_KEY = "6c4b684b76746d64383970706e4f74"  # 서울시 지하철 교통 데이터 API 키 (빈칸 채우기)
...
def get_bus_data(date):
    ...
    for row in root.findall('.//row'):
        bus_data.append({
            ...
            '노선번호': row.find('RTM_NM').text if row.find('RTM_NM') is not None else '',  # row에서 'RTE_NM' 태그의 텍스트 추출)
...
def get_subway_data(date):
    ...
    url = f"{SUBWAY_URL}{SUBWAY_API_KEY}/xml/CardSubwayStatsNew/1/1000/{date}"  # SUBWAY_URL과 SUBWAY_API_KEY, date를 이용하여 API 요청 URL 생성)
...
def integrate_data(bus_data, subway_data):
    ...
    return pd.DataFrame(subway_data)  # 최종적으로 '교통수단', '날짜', '노선번호', '역/정류장명', '승차인원', '하차인원' 컬럼만 포함한 데이터프레임 반환)
